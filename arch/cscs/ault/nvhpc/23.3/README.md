...
A100 node (ault25) with NVHPC 23.3.

## Prerequisites

- ault account with SLURM allocation (e.g. `-A g34`).
- Site NVHPC on ault only goes up to 21.3 (see `../21.3/`). This dir
  spack-installs NVHPC 23.3 from the NVIDIA tarball. First `spack install`
  therefore pulls ~4 GB and takes ~1 h.
...
```bash
spack env activate cloudsc-gpu
source ./arch/cscs/ault/nvhpc/23.3/env.sh
./cloudsc-bundle create
./cloudsc-bundle build --clean --with-gpu \
    --arch=./arch/cscs/ault/nvhpc/23.3 \
...
`build_all.sh` builds all three, but FP16 on ault + NVHPC 23.3 has not
...
| `Corrupt or Old Module file hdf5.mod` | HDF5 built with wrong compiler. Recreate spack env; `spack.yaml` pins `%nvhpc@23.3`. |
| `spack install nvhpc@23.3` fails to fetch | NVIDIA sometimes gates the tarball behind a login-wall. Check `spack info nvhpc` for the current URL, or fall back to `../21.3/spack.yaml`. |
