...
MODULE CLOUDSC_GLOBAL_STATE_MOD
  ...
  USE PARKIND1,  ONLY : JPIM, JPRB, JPRL, JPRD
  ...
  USE EXPAND_MOD, ONLY: LOAD_AND_EXPAND, LOAD_AND_EXPAND_STATE, &
    & LOAD_AND_EXPAND_R2_DPRD, LOAD_AND_EXPAND_R3_DPRD
  ...
  TYPE CLOUDSC_GLOBAL_STATE
    ...
    REAL(KIND=JPRL), ALLOCATABLE :: PAP(:,:,:)      ! Pressure on full levels  ! SC2026: JPRL — value(s) exceed FP16 max
    REAL(KIND=JPRL), ALLOCATABLE :: PAPH(:,:,:)     ! Pressure on half levels  ! SC2026: JPRL — value(s) exceed FP16 max
    ...
    REAL(KIND=JPRL), ALLOCATABLE :: PCLV(:,:,:,:)  ! SC2026: JPRL — values underflow FP16 (1e-6..1e-10 kg/kg)
...
!$omp end parallel do 
  ...
  SUBROUTINE CLOUDSC_GLOBAL_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG)
    ...
    ! SC2026: JPRD temp buffers — JPRB (FP16) would overflow/underflow
    REAL(KIND=JPRD), ALLOCATABLE :: TMP_PAP(:,:,:), TMP_PAPH(:,:,:)
    REAL(KIND=JPRD), ALLOCATABLE :: TMP_PCLV(:,:,:,:)
    ...
    ! SC2026: Load PAP/PAPH via JPRD path to avoid FP16 overflow (values up to ~101325 Pa > 65504)
    CALL LOAD_AND_EXPAND_R2_DPRD('PAP', TMP_PAP, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    ALLOCATE(SELF%PAP(NPROMA, SELF%KLEV, SELF%NBLOCKS))
    SELF%PAP = REAL(TMP_PAP, JPRL)
    DEALLOCATE(TMP_PAP)
    CALL LOAD_AND_EXPAND_R2_DPRD('PAPH', TMP_PAPH, KLON, SELF%KLEV+1, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    ALLOCATE(SELF%PAPH(NPROMA, SELF%KLEV+1, SELF%NBLOCKS))
    SELF%PAPH = REAL(TMP_PAPH, JPRL)
    DEALLOCATE(TMP_PAPH)
    ...
    ! SC2026: Load PCLV via JPRD path to avoid FP16 underflow (values 1e-6..1e-10 kg/kg)
    CALL LOAD_AND_EXPAND_R3_DPRD('PCLV', TMP_PCLV, KLON, SELF%KLEV, NCLV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    ALLOCATE(SELF%PCLV(NPROMA, SELF%KLEV, NCLV, SELF%NBLOCKS))
    SELF%PCLV = REAL(TMP_PCLV, JPRL)
    DEALLOCATE(TMP_PCLV)
