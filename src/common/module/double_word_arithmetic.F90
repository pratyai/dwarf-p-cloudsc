...
contains
  subroutine TwoSum(a, b, s, t)
    real(kind=hp), intent(in)  :: a, b; real(kind=hp), intent(out) :: s, t
    real(kind=hp) :: ap, bp, da, db; s = a + b; ap = s - b; bp = s - ap; da = a - ap; db = b - bp; t = da + db
  end subroutine
  subroutine Fast2Sum(a, b, s, t)
     real(kind=hp), intent(in)  :: a, b; real(kind=hp), intent(out) :: s, t
     real(kind=hp) :: z; s = a + b; z = s - a; t = b - z
  end subroutine
  subroutine TwoProd(a, b, p, r)
    real(kind=hp), intent(in)  :: a, b; real(kind=hp), intent(out) :: p, r
    p = a * b; r = ieee_fma(a, b, -p)
  end subroutine
  function negate_dw(a) result(res)
    type(double_word_hp), intent(in) :: a; type(double_word_hp) :: res; res%h=-a%h; res%l=-a%l
  end function
  function plus_dw_dw(a, b) result(res)
    type(double_word_hp), intent(in) :: a, b; type(double_word_hp) :: res
    real(kind=hp) :: sh, sl, th, tl, c, vh, vl, w
    call TwoSum(a%h, b%h, sh, sl); call TwoSum(a%l, b%l, th, tl); c = sl + th
    call Fast2Sum(sh, c, vh, vl); w = tl + vl; call Fast2Sum(vh, w, res%h, res%l)
  end function
  function plus_dw_fp(a, b) result(res)
    type(double_word_hp), intent(in) :: a; real(kind=hp), intent(in) :: b; type(double_word_hp) :: res
    real(kind=hp) :: sh, sl, v; call TwoSum(a%h, b, sh, sl); v = a%l + sl; call Fast2Sum(sh, v, res%h, res%l)
  end function
  function plus_fp_dw(a, b) result(res)
    real(kind=hp), intent(in) :: a; type(double_word_hp), intent(in) :: b; type(double_word_hp) :: res
    res = b + a
  end function
  function minus_dw_dw(a, b) result(res)
    type(double_word_hp), intent(in) :: a, b; type(double_word_hp) :: res; res = a + (-b)
  end function
  ...
  function times_dw_dw(a, b) result(res)
    type(double_word_hp), intent(in) :: a, b; type(double_word_hp) :: res
    real(kind=hp) :: ch, cl1, tl, cl2, cl3; call TwoProd(a%h, b%h, ch, cl1); tl=a%h*b%l
    cl2=ieee_fma(a%l,b%h,tl); cl3=cl1+cl2; call Fast2Sum(ch,cl3,res%h,res%l)
  end function
  function times_dw_fp(a, b) result(res)
    type(double_word_hp), intent(in) :: a; real(kind=hp), intent(in) :: b; type(double_word_hp) :: res
    real(kind=hp) :: ch, cl1, cl3; call TwoProd(a%h, b, ch, cl1); cl3=ieee_fma(a%l,b,cl1)
  ...
  function div_dw_fp(a, b) result(res)
    type(double_word_hp), intent(in) :: a; real(kind=hp), intent(in) :: b; type(double_word_hp) :: res
    real(kind=hp) :: th, ph, pl, dh, dt, d, tl; th=a%h/b; call TwoProd(th,b,ph,pl); dh=a%h-ph
    dt=dh-pl; d=dt+a%l; tl=d/b; call Fast2Sum(th,tl,res%h,res%l)
  end function
  function to_string_hp(a) result(s)
    type(double_word_hp), intent(in) :: a; character(len=80) :: s
...
contains
  subroutine TwoSum(a, b, s, t)
    real(kind=sp), intent(in)  :: a, b; real(kind=sp), intent(out) :: s, t
    real(kind=sp) :: ap, bp, da, db; s = a + b; ap = s - b; bp = s - ap; da = a - ap; db = b - bp; t = da + db
  end subroutine
  subroutine Fast2Sum(a, b, s, t)
     real(kind=sp), intent(in)  :: a, b; real(kind=sp), intent(out) :: s, t
     real(kind=sp) :: z; s = a + b; z = s - a; t = b - z
  end subroutine
  subroutine TwoProd(a, b, p, r)
    real(kind=sp), intent(in)  :: a, b; real(kind=sp), intent(out) :: p, r
    p = a * b; r = ieee_fma(a, b, -p)
  end subroutine
  function negate_dw(a) result(res)
    type(double_word_sp), intent(in) :: a; type(double_word_sp) :: res; res%h=-a%h; res%l=-a%l
  end function
  function plus_dw_dw(a, b) result(res)
    type(double_word_sp), intent(in) :: a, b; type(double_word_sp) :: res
    real(kind=sp) :: sh, sl, th, tl, c, vh, vl, w
    call TwoSum(a%h, b%h, sh, sl); call TwoSum(a%l, b%l, th, tl); c = sl + th
    call Fast2Sum(sh, c, vh, vl); w = tl + vl; call Fast2Sum(vh, w, res%h, res%l)
  end function
  function plus_dw_fp(a, b) result(res)
    type(double_word_sp), intent(in) :: a; real(kind=sp), intent(in) :: b; type(double_word_sp) :: res
    real(kind=sp) :: sh, sl, v; call TwoSum(a%h, b, sh, sl); v = a%l + sl; call Fast2Sum(sh, v, res%h, res%l)
  end function
  function plus_fp_dw(a, b) result(res)
    real(kind=sp), intent(in) :: a; type(double_word_sp), intent(in) :: b; type(double_word_sp) :: res
    res = b + a
  end function
  function minus_dw_dw(a, b) result(res)
    type(double_word_sp), intent(in) :: a, b; type(double_word_sp) :: res; res = a + (-b)
  end function
  function minus_dw_fp(a, b) result(res)
    type(double_word_sp), intent(in) :: a; real(kind=sp), intent(in) :: b; type(double_word_sp) :: res
    res = a + (-b)
  ...
  function times_dw_dw(a, b) result(res)
    type(double_word_sp), intent(in) :: a, b; type(double_word_sp) :: res
    real(kind=sp) :: ch, cl1, tl, cl2, cl3; call TwoProd(a%h, b%h, ch, cl1); tl=a%h*b%l
    cl2=ieee_fma(a%l,b%h,tl); cl3=cl1+cl2; call Fast2Sum(ch,cl3,res%h,res%l)
  end function
  function times_dw_fp(a, b) result(res)
    type(double_word_sp), intent(in) :: a; real(kind=sp), intent(in) :: b; type(double_word_sp) :: res
    real(kind=sp) :: ch, cl1, cl3; call TwoProd(a%h, b, ch, cl1); cl3=ieee_fma(a%l,b,cl1)
    call Fast2Sum(ch, cl3, res%h, res%l)
  end function
  function times_fp_dw(a, b) result(res)
    real(kind=sp), intent(in) :: a; type(double_word_sp), intent(in) :: b; type(double_word_sp) :: res
  ...
  function div_dw_fp(a, b) result(res)
    type(double_word_sp), intent(in) :: a; real(kind=sp), intent(in) :: b; type(double_word_sp) :: res
    real(kind=sp) :: th, ph, pl, dh, dt, d, tl; th=a%h/b; call TwoProd(th,b,ph,pl); dh=a%h-ph
    dt=dh-pl; d=dt+a%l; tl=d/b; call Fast2Sum(th,tl,res%h,res%l)
  end function
  function to_string_sp(a) result(s)
    type(double_word_sp), intent(in) :: a; character(len=80) :: s
...
contains
  subroutine TwoSum(a, b, s, t)
    real(kind=dp), intent(in)  :: a, b; real(kind=dp), intent(out) :: s, t
    real(kind=dp) :: ap, bp, da, db; s = a + b; ap = s - b; bp = s - ap; da = a - ap; db = b - bp; t = da + db
  end subroutine
  subroutine Fast2Sum(a, b, s, t)
     real(kind=dp), intent(in)  :: a, b; real(kind=dp), intent(out) :: s, t
     real(kind=dp) :: z; s = a + b; z = s - a; t = b - z
  end subroutine
  subroutine TwoProd(a, b, p, r)
    real(kind=dp), intent(in)  :: a, b; real(kind=dp), intent(out) :: p, r
    p = a * b; r = ieee_fma(a, b, -p)
  end subroutine
  function negate_dw(a) result(res)
    type(double_word_dp), intent(in) :: a; type(double_word_dp) :: res; res%h=-a%h; res%l=-a%l
  end function
  function plus_dw_dw(a, b) result(res)
    type(double_word_dp), intent(in) :: a, b; type(double_word_dp) :: res
    real(kind=dp) :: sh, sl, th, tl, c, vh, vl, w
    call TwoSum(a%h, b%h, sh, sl); call TwoSum(a%l, b%l, th, tl); c = sl + th
    call Fast2Sum(sh, c, vh, vl); w = tl + vl; call Fast2Sum(vh, w, res%h, res%l)
  end function
  function plus_dw_fp(a, b) result(res)
    type(double_word_dp), intent(in) :: a; real(kind=dp), intent(in) :: b; type(double_word_dp) :: res
    real(kind=dp) :: sh, sl, v; call TwoSum(a%h, b, sh, sl); v = a%l + sl; call Fast2Sum(sh, v, res%h, res%l)
  end function
  function plus_fp_dw(a, b) result(res)
    real(kind=dp), intent(in) :: a; type(double_word_dp), intent(in) :: b; type(double_word_dp) :: res
    res = b + a
  end function
  function minus_dw_dw(a, b) result(res)
    type(double_word_dp), intent(in) :: a, b; type(double_word_dp) :: res; res = a + (-b)
  end function
  function minus_dw_fp(a, b) result(res)
    type(double_word_dp), intent(in) :: a; real(kind=dp), intent(in) :: b; type(double_word_dp) :: res
    res = a + (-b)
  ...
  function times_dw_dw(a, b) result(res)
    type(double_word_dp), intent(in) :: a, b; type(double_word_dp) :: res
    real(kind=dp) :: ch, cl1, tl, cl2, cl3; call TwoProd(a%h, b%h, ch, cl1); tl=a%h*b%l
    cl2=ieee_fma(a%l,b%h,tl); cl3=cl1+cl2; call Fast2Sum(ch,cl3,res%h,res%l)
  end function
  function times_dw_fp(a, b) result(res)
    type(double_word_dp), intent(in) :: a; real(kind=dp), intent(in) :: b; type(double_word_dp) :: res
    real(kind=dp) :: ch, cl1, cl3; call TwoProd(a%h, b, ch, cl1); cl3=ieee_fma(a%l,b,cl1)
    call Fast2Sum(ch, cl3, res%h, res%l)
  end function
  function times_fp_dw(a, b) result(res)
    real(kind=dp), intent(in) :: a; type(double_word_dp), intent(in) :: b; type(double_word_dp) :: res
  ...
  function div_dw_fp(a, b) result(res)
    type(double_word_dp), intent(in) :: a; real(kind=dp), intent(in) :: b; type(double_word_dp) :: res
    real(kind=dp) :: th, ph, pl, dh, dt, d, tl; th=a%h/b; call TwoProd(th,b,ph,pl); dh=a%h-ph
    dt=dh-pl; d=dt+a%l; tl=d/b; call Fast2Sum(th,tl,res%h,res%l)
  end function
  function to_string_dp(a) result(s)
    type(double_word_dp), intent(in) :: a; character(len=80) :: s
...
contains
  ...
  subroutine convert_dp_to_dw_hp(dp_array, dw_hp_array)
    ...
    integer :: i, j, k; real(kind=dp) :: dp_val; real(kind=hp) :: h_part, l_part
    if (any(shape(dw_hp_array) /= shape(dp_array))) error stop "convert_dp_to_dw_hp: Shapes do not match."
    do k = 1, size(dp_array, 3); do j = 1, size(dp_array, 2); do i = 1, size(dp_array, 1)
      dp_val = dp_array(i,j,k); h_part = real(dp_val, kind=hp)
  ...
  subroutine convert_dp_to_dw_sp(dp_array, dw_sp_array)
    ...
    integer :: i, j, k; real(kind=dp) :: dp_val; real(kind=sp) :: h_part, l_part
    if (any(shape(dw_sp_array) /= shape(dp_array))) error stop "convert_dp_to_dw_sp: Shapes do not match."
    do k = 1, size(dp_array, 3); do j = 1, size(dp_array, 2); do i = 1, size(dp_array, 1)
      dp_val = dp_array(i,j,k); h_part = real(dp_val, kind=sp)
  ...
  subroutine convert_dw_hp_to_dp(dw_hp_array, dp_array)
    ...
    integer :: i, j, k; type(double_word_hp) :: dw_val
    if (any(shape(dp_array) /= shape(dw_hp_array))) error stop "convert_dw_hp_to_dp: Shapes do not match."
    do k = 1, size(dw_hp_array, 3); do j = 1, size(dw_hp_array, 2); do i = 1, size(dw_hp_array, 1)
  ...
  subroutine convert_dw_sp_to_dp(dw_sp_array, dp_array)
    ...
    real(kind=dp), dimension(:,:,:), intent(inout)   :: dp_array
    integer :: i, j, k; type(double_word_sp) :: dw_val
    if (any(shape(dp_array) /= shape(dw_sp_array))) error stop "convert_dw_sp_to_dp: Shapes do not match."
    do k = 1, size(dw_sp_array, 3); do j = 1, size(dw_sp_array, 2); do i = 1, size(dw_sp_array, 1)
  ...
  subroutine convert_dw_dp_to_dp(dw_dp_array, dp_array)
    ...
    real(kind=dp), dimension(:,:,:), intent(inout)   :: dp_array
    integer :: i, j, k; type(double_word_dp) :: dw_val
    if (any(shape(dp_array) /= shape(dw_dp_array))) error stop "convert_dw_dp_to_dp: Shapes do not match."
    do k = 1, size(dw_dp_array, 3); do j = 1, size(dw_dp_array, 2); do i = 1, size(dw_dp_array, 1)
