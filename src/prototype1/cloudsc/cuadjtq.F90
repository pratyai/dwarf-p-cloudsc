...
!       ZQSAT=FOEEWMCU(PT(JL,KK))*ZQP    
        ...
        ZL=1.0_JPRB/(ZZPT(JJ)-R4LES)
        ZI=1.0_JPRB/(ZZPT(JJ)-R4IES)
        ZQSAT=R2ES *(FOEALFCU(ZZPT(JJ))*EXP(R3LES*(ZZPT(JJ)-RTT)*ZL)+&
          &(1.0_JPRB-FOEALFCU(ZZPT(JJ)))*EXP(R3IES*(ZZPT(JJ)-RTT)*ZI))
        ZQSAT=ZQSAT*ZQP
...
!       ZCOND=(PQ(JL,KK)*ZCOR**2-ZQSAT*ZCOR)/(ZCOR**2+ZQSAT*FOEDEMCU(PT(JL,KK)))
        ...
        ZF=FOEALFCU(ZZPT(JJ))*R5ALVCP*ZL**2 + &
          &(1.0_JPRB-FOEALFCU(ZZPT(JJ)))*R5ALSCP*ZI**2
        ZCOND=(ZZPQ(JJ)*ZCOR**2-ZQSAT*ZCOR)/(ZCOR**2+ZQSAT*ZF)
...
!         ZQP=ZZQP(JJ)
          ZZPT(JJ)=ZZPT(JJ)+FOELDCPMCU(ZZPT(JJ))*ZCOND
          ZZPQ(JJ)=ZZPQ(JJ)-ZCOND
!         ZQSAT=FOEEWMCU(ZZPT(JJ))*ZQP    
          ZL=1.0_JPRB/(ZZPT(JJ)-R4LES)
          ZI=1.0_JPRB/(ZZPT(JJ)-R4IES)
          ZQSAT=R2ES *(FOEALFCU(ZZPT(JJ))*EXP(R3LES*(ZZPT(JJ)-RTT)*ZL)+&
            &(1.0_JPRB-FOEALFCU(ZZPT(JJ)))*EXP(R3IES*(ZZPT(JJ)-RTT)*ZI))
...
!         ZCOND1=(PQ(JL,KK)*ZCOR**2-ZQSAT*ZCOR)/(ZCOR**2+ZQSAT*FOEDEMCU(PT(JL,KK)))
          ZF=FOEALFCU(ZZPT(JJ))*R5ALVCP*ZL**2 + &
            &(1.0_JPRB-FOEALFCU(ZZPT(JJ)))*R5ALSCP*ZI**2
          ZCOND1=(ZZPQ(JJ)*ZCOR**2-ZQSAT*ZCOR)/(ZCOR**2+ZQSAT*ZF)
          IF(ZCOND ==  0.0_JPRB)ZCOND1=0.0_JPRB
          ZZPT(JJ)=ZZPT(JJ)+FOELDCPMCU(ZZPT(JJ))*ZCOND1
